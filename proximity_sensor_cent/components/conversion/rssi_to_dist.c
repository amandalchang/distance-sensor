...
static void shift_out_msb(gpio_num_t dataPin, gpio_num_t clockPin,
                          uint8_t val) {
  for (int i = 0; i < 8; i++) {
...
static int get_average_rssi(float* avg_rssi, const int measure_count,
                            const int task_delay_ms) {
  float sum = 0.0f;
  int8_t rssi_val;
  int valid_count = 0;
