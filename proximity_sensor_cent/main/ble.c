...
static int ble_prox_cent_on_read(uint16_t conn_handle,
                                 const struct ble_gatt_error* error,
                                 struct ble_gatt_attr* attr, void* arg) {
  MODLOG_DFLT(INFO,
              "Read on tx power level char completed; status=%d "
              "conn_handle=%d\n",
 ...
 */
static int ble_prox_cent_on_write(uint16_t conn_handle,
                                  const struct ble_gatt_error* error,
                                  struct ble_gatt_attr* attr, void* arg) {
  MODLOG_DFLT(INFO,
              "Write alert level char completed; status=%d conn_handle=%d",
              error->status, conn_handle);
  ...
  chr = peer_chr_find_uuid(
      peer, BLE_UUID16_DECLARE(BLE_SVC_TX_POWER_UUID16),
      BLE_UUID16_DECLARE(BLE_SVC_PROX_CHR_UUID16_TX_PWR_LVL));
  if (chr == NULL) {
    MODLOG_DFLT(ERROR,
                "Error: Peer doesn't support the"
                "Tx power level characteristic\n");
    goto err;
  }

  rc = ble_gattc_read(conn_handle, chr->chr.val_handle, ble_prox_cent_on_read,
                      NULL);
  if (rc != 0) {
...
static void ble_prox_cent_read_write_subscribe(const struct peer* peer) {
  ...
  rc = ble_gattc_write_flat(
      peer->conn_handle, chr->chr.val_handle, &link_supervision_timeout,
      sizeof(link_supervision_timeout), ble_prox_cent_on_write, NULL);
  if (rc != 0) {
 ...
 */
static void ble_prox_cent_on_disc_complete(const struct peer* peer, int status,
                                           void* arg) {
  if (status != 0) {
 ...
 */
static int ble_prox_cent_should_connect(const struct ble_gap_disc_desc* disc) {
  ...
  if (strlen(CONFIG_EXAMPLE_PEER_ADDR) &&
      (strncmp(CONFIG_EXAMPLE_PEER_ADDR, "ADDR_ANY", strlen("ADDR_ANY")) !=
       0)) {
    ESP_LOGI(tag, "Peer address from menuconfig: %s", CONFIG_EXAMPLE_PEER_ADDR);
    /* Convert string to address */
    sscanf(CONFIG_EXAMPLE_PEER_ADDR, "%lx:%lx:%lx:%lx:%lx:%lx", &peer_addr[5],
 ...
 */
static void ble_prox_cent_connect_if_interesting(void* disc) {
  uint8_t own_addr_type;
  int rc;
  ble_addr_t* addr;
  ...
  switch (event->type) {
      ...
      if (event->connect.status == 0) {
        ...
        for (int i = 0; i <= MYNEWT_VAL(BLE_MAX_CONNECTIONS); i++) {
          if (disconn_peer[i].addr != NULL) {
            if (memcmp(disconn_peer[i].addr, &desc.peer_id_addr.val,
                       BLE_ADDR_LEN)) {
    ...
    case BLE_GAP_EVENT_DISCONNECT:
      ...
      for (int i = 0; i <= MYNEWT_VAL(BLE_MAX_CONNECTIONS); i++) {
        if (disconn_peer[i].addr == NULL) {
          disconn_peer[i].addr =
              (uint8_t*)malloc(BLE_ADDR_LEN * sizeof(uint8_t));
          if (disconn_peer[i].addr == NULL) {
      ...
      /* Stop calculating path loss, restart once connection is established
       * again */
    ...
    case BLE_GAP_EVENT_ENC_CHANGE:
      ...
      //           MODLOG_DFLT(INFO, "cache association; conn_handle=%d
      //           status=%d cache_state=%s\n",
      //                       event->cache_assoc.conn_handle,
      //                       event->cache_assoc.status,
      //                       (event->cache_assoc.cache_state == 0) ? "INVALID"
      //                       : "LOADED");
      //           /* Perform service discovery */
      //           rc = peer_disc_all(event->connect.conn_handle,
      //                              blecent_on_disc_complete, NULL);
      //           if(rc != 0) {
      //                 MODLOG_DFLT(ERROR, "Failed to discover services;
      //                 rc=%d\n", rc); return 0;
      //           }
      // #endif
      //   return 0;
...
static void ble_prox_cent_on_reset(int reason) {
  MODLOG_DFLT(ERROR, "Resetting state; reason=%d\n", reason);
}

static void ble_prox_cent_on_sync(void) {
  int rc;
...
static void ble_prox_cent_init(void) {
  /* Task for alerting when link is lost */
  xTaskCreate(ble_prox_cent_link_loss_task, "ble_prox_cent_link_loss_task",
              4096, NULL, 10, NULL);
  return;
}
