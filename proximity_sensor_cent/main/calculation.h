 ...
 * @param rssi (const) The float of an RSSI value averaged.
 * @param m_val (const) The float slope of the linear regression model.
 * @param b_val (const) The float y-intercept of the linear regression model.
 *
 * @return The int 0 if conversion was successful, int 1 if not.
 */
int rssi_to_dist(float* dist, float rssi, float m_val, float b_val);
 ...
 *
 * @param num_dists (const) The int number of distances calibrated at.
 * @param rssi_array The float array with elements representing
 *  averaged RSSI values at each calibration location.
 * @param m_val The pointer to a float variable.
 * @param b_val The pointer to a float variable.
 */
void rssi_logreg_to_params(int num_dists, const float* rssi_array,
